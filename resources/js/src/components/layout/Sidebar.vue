          ...
          <ul class="relative font-semibold space-y-0.5 p-4 py-0">
              ...
              <router-link to="/" class="group" @click="toggleMobileMenu">
                <div class="flex items-center">
                  ...
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
                    class="group-hover:!text-primary shrink-0 size-6">
            ...
            <li class="nav-item">
              <router-link to="/submission" class="group" @click="toggleMobileMenu">
                <div class="flex items-center">
                  <svg class="group-hover:!text-primary shrink-0 size-6" xmlns="http://www.w3.org/2000/svg" fill="none"
            ...
            <li class="menu nav-item">
              <button type="button" class="nav-link group w-full" :class="{ active: activeDropdown === 'approval' }"
                ...
                <div class="flex items-center">
                  ...
                  <span
                    class="ltr:pl-3 rtl:pr-3 text-black dark:text-[#506690] dark:group-hover:text-white-dark flex items-center gap-2">
                    {{ $t('Persetujuan') }}
                    <span v-if="approvalCount?.total > 0"
                      class="inline-flex items-center justify-center px-2 py-1 text-[10px] font-bold leading-none text-white bg-red-500 rounded-full">
                      {{ approvalCount?.total }}
                    </span>
                  </span>
                </div>
                <div class="rtl:rotate-180"
                  :class="{ '!rotate-90': activeDropdown === 'approval' || isPRApprovalActive || isPOApprovalActive || isRAApprovalActive }">
              ...
              <transition>
                <div v-show="activeDropdown === 'approval' || isPRApprovalActive || isPOApprovalActive || isRAApprovalActive"
                  class="overflow-hidden">
                  <ul class="sub-menu text-gray-500">
                    <li>
                      <router-link to="/approval/pr-approval" @click="toggleMobileMenu" class='flex gap-2'
                        :class="{ 'active': isPRApprovalActive }">
                        Persetujuan PR
                        <span v-if="approvalCount?.pr > 0"
                          class="inline-flex items-center justify-center px-2 py-1 text-[10px] font-bold leading-none text-white bg-red-500 rounded-full">
                          {{ approvalCount.pr }}
                        </span>
                      </router-link>
                    ...
                    <li>
                      <router-link to="/approval/po-approval" @click="toggleMobileMenu" class='flex gap-2'
                        :class="{ 'active': isPOApprovalActive }">
                        Persetujuan PO
                        <span v-if="approvalCount?.po > 0"
                          class="inline-flex items-center justify-center px-2 py-1 text-[10px] font-bold leading-none text-white bg-red-500 rounded-full">
                          {{ approvalCount.po }}
                        </span>
                      </router-link>
                    ...
                    <li>
                      <router-link to="/approval/ra-approval" @click="toggleMobileMenu" class='flex gap-2'
                        :class="{ 'active': isRAApprovalActive }">
                        Persetujuan RA
                        <span v-if="approvalCount?.ra > 0"
                          class="inline-flex items-center justify-center px-2 py-1 text-[10px] font-bold leading-none text-white bg-red-500 rounded-full">
                          {{ approvalCount.ra }}
                        </span>
                      </router-link>
...
import { useCountApproval } from '@/services/queries';
...
const { data: approvalCount } = useCountApproval(store.user?.username || '');
