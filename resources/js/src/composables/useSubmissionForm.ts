...
import { separateNominal } from "./separate";

export function useSubmissionForm({ submissionRef, formType, formData, defaultFormData, isDraft, approvalLayers, selectedPR, selectedPO, currentUser, userDataRef, notedDataRef, notesHistory }) {
    ...
    watchEffect(() => {
        ...
        console.log("Current User in useSubmissionForm:", currentUser.value.username, data.created_by);

        // Kondisi per tipe form
        if (formType.value === "purchase-request") {
            isDraft.value = (data.status === "Draft" || data.status === "Revised") && data.created_by == currentUser.value.username;
            // --- usePurchaseRequest ---
            ...
            formData.value.jenisPermintaan = data.jenis_permintaan || "";
            ...
            if (Array.isArray(data.purchase_request_items) && data.purchase_request_items.length > 0) {
                formData.value.items = data.purchase_request_items.map((item: any) => ({
            ...
            if (data.status.toLowerCase() !== "draft") {
                ...
                formData.value.justification = data.purchase_request.justification;
                formData.value.po_date = data.po_date;
                formData.value.pr_date = data.purchase_request.created_at;
                ...
                approvalLayers.value = data.approvals;
                formData.value.items = data.purchase_order_items.map((item: any) => ({
                    item_id: item.item_master.id || "",
                    quantity: item.quantity || 1,
                    pengajuan: item.pengajuan || "",
                    category: item.item_master.category.nama_katbrg || "",
            ...
            if (data.purchase_order_number !== null && data.status !== "Draft") {
                ...
                formData.value.jenisPermintaan = data.purchase_order.purchase_request.jenis_permintaan || "";
                formData.value.cabang = data.purchase_order.purchase_request.cabang || "";
                formData.value.requestedBy = data.purchase_order.purchase_request.created_by || "";
                formData.value.department = data.purchase_order.purchase_request.department || "";
                formData.value.justification = data.purchase_order.purchase_request.justification || "";
                formData.value.invoice_date = data.invoice_date || "";
                formData.value.ra_date = data.ra_date || "";
                formData.value.requestDate = "";
                formData.value.poReference = data.purchase_order_number || "";
                formData.value.prReference = data.purchase_order.purchase_request.pr_number;
                formData.value.vendor_id = data.purchase_order.vendor_id || "";
                formData.value.status = data.status || "Draft";
                formData.value.notes = data.notes || [];
                approvalLayers.value = data.approvals;
                if (Array.isArray(data.registration_asset_items) && data.registration_asset_items.length > 0) {
                    formData.value.items = data.registration_asset_items.map((item: any) => ({
                        item_id: item.item_master.id || "",
                        quantity: item.quantity || 1,
                        pengajuan: item.pengajuan || "",
                        category: item.item_master.category.nama_katbrg || "",
            ...
            } else if (data.purchase_order_number !== null && data.status === "Draft") {
                console.log("registration asset loaded : ", data);
                isDraft.value = (data.status === "Draft" || data.status === "Revised") && data.created_by == currentUser.value.username;
                ...
                formData.value.jenisPermintaan = data.purchase_order.purchase_request.jenis_permintaan || "";
                formData.value.cabang = data.purchase_order.purchase_request.cabang || "";
                formData.value.po_date = data.purchase_order.po_date || "";
                formData.value.requestedBy = data.purchase_order.purchase_request.created_by || "";
                formData.value.department = data.purchase_order.purchase_request.department || "";
                formData.value.justification = data.purchase_order.purchase_request.justification || "";
